...
Completing these points are essential before we go deeper into project management topics in @sec-openscience and @sec-codemgmt. @sec-programmingprimers will focus on how to code.
...
- Keep `.R` files that implement a user-defined function (see @sec-programmingprimers) in a sub-directory `./R/`, and `.R` files that implement a workflow in a sub-directory `./analysis/`. Workflow implementations with a view to communication should be implemented in RMarkdown files.
...
We will learn more about *git* in @sec-codemgmt. For now, follow instructions in Section @sec-setup to set yourself up with installing *git* and other required stuff on your computer and with a GitHub account. You will use this later in this course (and maybe throughout your career).
