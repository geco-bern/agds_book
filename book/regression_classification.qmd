...
To fit a univariate linear regression model in R, we can use the `lm()` function. Already in @sec-datawrangling, we created linear models by doing:
...
plot_5 <- ggplot(
  ...
  labs(
    title = "Histogram, density and boxplot",
...
plot_10 <- ggplot(
  ...
  geom_qq_line() +
  labs(
    y = expression(paste("Shortwave radiation (W m"^-2, ")")),
    x = "Theoretical normal quantiles"
  ) +
  theme_classic()
...
datasets::CO2 |>
  ...
  geom_point(size = 3, alpha = 0.5) +
  labs(
    x = expression(paste("Uptake (", mu, "mol CO"[2], " m"^-2, "s"^-1, ")")),
    y = ""
  ) +
  theme_classic() +
  ...
  stat_smooth(
    method = "glm", color = "grey", se = FALSE,
...
1. An evaluation of all bivariate models (single predictor), implementing just steps 1-3 of the algorithm described in @sec-variableselection. This should be complemented by a visualisation and a brief discussion of the results.
...
- Remember the structure of the loops implemented for the Exercise *Nested loops* in @sec-primers.
